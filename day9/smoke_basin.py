...
def generate_neighbormap(heightmap):
    ...
    for row_idx, row in enumerate(heightmap):
        ...
        for col_idx in range(len(row)):
            valid_neighbor_coords = get_valid_neighbor_coords(row_idx, col_idx, len(heightmap), len(row))
            neighbormap[row_idx][col_idx] = [heightmap[coord.r][coord.c] for coord in valid_neighbor_coords]
    return neighbormap
...
def find_basins_sizes(local_mins_coord, heightmap):
    ...
    for local_min in local_mins_coord:
        ...
        while len(seed) > 0:
            ...
            if current_coord not in processed and \
               heightmap[current_coord.r][current_coord.c] < 9:

                valid_neighbor_coords = get_valid_neighbor_coords(current_coord.r, current_coord.c, len_row, len_col)
            ...
            elif current_coord not in processed and \
               heightmap[current_coord.r][current_coord.c] == 9:

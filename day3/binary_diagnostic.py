...
def get_epsilon_rate(diagnostic_report):
    diagnostic_report_transpose = list(map(list, zip(*diagnostic_report)))
    epsilon_rate_str_list = ["0" if nst_digit.count("0") < nst_digit.count("1") else "1" for nst_digit in diagnostic_report_transpose]
...
def get_oxygen_generator_rating(diagnostic_report):
    ...
    while idx < len(diagnostic_report[0]):
        ...
        nst_digit = [diagnostic_record[idx] for diagnostic_record in filtered_diagnostic_report]
        most_common_value = "0" if nst_digit.count("0") > nst_digit.count("1") else "1"  # if "0" and "1" are equally common, keep values with a "1" in the position being considered.
        logger.debug(f"most common value: '{most_common_value}'")

        filtered_diagnostic_report = list(filter(lambda diagnostic_record: diagnostic_record[idx] == most_common_value, filtered_diagnostic_report))
...
def get_co2_scrubber_rating(diagnostic_report):
    ...
    while idx < len(diagnostic_report[0]):
        ...
        nst_digit = [diagnostic_record[idx] for diagnostic_record in filtered_diagnostic_report]
        most_common_value = "1" if nst_digit.count("1") < nst_digit.count("0") else "0"  # if "0" and "1" are equally common, keep values with a "0" in the position being considered.
        logger.debug(f"most common value: '{most_common_value}'")
